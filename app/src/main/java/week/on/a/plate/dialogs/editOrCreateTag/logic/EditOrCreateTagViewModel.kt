...
import week.on.a.plate.dialogs.editOrCreateTag.state.EditOrCreateTagUIState
...
class EditOrCreateTagViewModel(
    ...
    val state: EditOrCreateTagUIState = EditOrCreateTagUIState()

    init {
        state.text.value = oldName ?: ""
        state.category.value = oldCategory ?: defaultCategoryView
        state.isForCreate.value = oldName == null
    }


    fun onEvent(event: EditOrCreateTagEvent) {
        when (event) {
            EditOrCreateTagEvent.Close -> close()
            EditOrCreateTagEvent.Done -> {
                if (state.category.value == null) return
                val result = Pair(state.text.value, state.category.value!!)
                done(result)
            }

            EditOrCreateTagEvent.ChooseCategory -> toSearchCategory()
        }
    }
