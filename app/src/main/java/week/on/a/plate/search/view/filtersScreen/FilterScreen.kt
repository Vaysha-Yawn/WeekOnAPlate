...
@Composable
fun FilterScreen(stateUI: FilterUIState, onEvent: (Event) -> Unit) {
...
fun IngredientCategories(
    ingredient: IngredientCategoryView,
    selectedIngredients: MutableState<List<IngredientView>>
) {
    TextTitle(text = ingredient.name)
    Spacer(modifier = Modifier.height(12.dp))
    for (ingredientd in ingredient.ingredientViews) {
        TagBig(ingredientView = ingredientd, selectedIngredients.value.contains(ingredientd))
        Spacer(modifier = Modifier.height(12.dp))
    }
}
