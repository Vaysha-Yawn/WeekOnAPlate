...
@Composable
fun EditOrCreateIngredient(
    state: EditOrCreateIngredientUIState,
    onEvent: (EditOrCreateIngredientEvent) -> Unit,
) {
    ...
    LazyColumn(modifier = Modifier
        ...
        item {
            ...
            CommonButton(
                ...
                onEvent(EditOrCreateIngredientEvent.ChooseCategory)
            }
            Spacer(modifier = Modifier.height(24.dp))
            TextBody(
            ...
            CommonButton(stringResource(R.string.select_photo)) {
                onEvent(EditOrCreateIngredientEvent.PickImage)
            }
            Spacer(modifier = Modifier.height(24.dp))
            ...
            DoneButton(
                ...
                if (state.name.value != "" && state.category.value != null) {
                    onEvent(EditOrCreateIngredientEvent.Done(context))
                } else {
                    isError.value = true
                }
            }
...
fun PreviewAddIngredient() {
    WeekOnAPlateTheme {
        val state = EditOrCreateIngredientUIState("", false, null, "", false)
        EditOrCreateIngredient(state) {}
    }
}